...
It will always try the re-apply the most recent successful format first. For example, if an entry has a timestamp \code{02 Jan 06 15:04 MST}, the ingester will attempt to parse the \emph{next} entry with the same timestamp format. If it does not match, then the ingester will attempt all other timestamp formats.
