...
Gravwell is not a bit field index which means that it will only accelerate on direct matches.  For example \code{tag=syslog syslog Hostname=="foobar"} can invoke the accelerator, but \code{tag=syslog ProcID < 20} will not.  The accelerators also do not accelerate on the negative, meaning that \code{tag=syslog syslog Hostname != "foobar"} will not invoke the accelerator engine.
