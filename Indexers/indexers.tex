...
The \code{Required-Retention} stanza can specify a period that directs the configured
...
The \code{Required-Retention=30d} also informs the well that it is not allowed
...
\textbf{WARNING}: The \code{Required-Retention} flag will check specified retention
periods against time based ageout. Setting \code{Required-Retention=90d} and
\code{Cold-Duration=80d} will result in an error as the required retention
...
Every indexer can define a single Cloud Archive configuration block which specifies the remote archive server and authentication token. The configuration block is specified using the \code{[Cloud-Archive]} section header.  To enable Cloud Archive on a well, add the \code{Archive-Deleted-Shards=true} directive within that well.
...
Use the \code{gravwell\_cloudarchive\_usertool} command to set up the password database with an entry for your customer number:
...
A default configuration will be installed at \code{/opt/gravwell/etc/cloudarchive\_server.conf}. This configuration stores archived shards on the local disk, in \code{/opt/gravwell/cloudarchive/}. It listens for clients on port 8886, using the specified TLS cert/key pair for encryption. The \code{Password-File} parameter points at the password database set up earlier.
...
Note: If you don't want to set up certificates for TLS, you can put the server into plaintext mode by setting \code{Disable-TLS=true}. Be aware that this is horribly insecure and a terrible idea unless your Cloud Archive server and your indexers are on the same trusted network!

\subsubsection{Configure Gravwell}

Configure your Gravwell indexers as above, setting the \code{Cloud-Archive} stanza to point at your server. The \code{Archive-Shared-Secret} value should match the password you entered when running \code{gravwell\_cloudarchive\_usertool}.
...
If you disabled TLS on the server, set \code{Insecure-Disable-TLS=true} in the \code{Cloud-Archive} stanza. If you are using self-signed certs, set \code{Insecure-Skip-TLS-Verify=true}.
...
\textbf{NOTE:} Replication requires a paid Gravwell license. Community Edition licenses do not support replication. Before beginning this lab, verify that your license has replication enabled. If replication is not working despite a correct configuration, check your license.
...
[Storage-Well "json"]
    ...
    Accelerator-Args="user email country group useragent ip"
\end{Verbatim}
...
While both the \code{useragent} and \code{country} fields are specified in the
...
running. The manager application is an open-source\footnote{https://github.com/gravwell/gravwell/tree/main/manager} Gravwell
