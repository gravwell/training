...
The ``other'' data group is a combined calculation of all remaining data series that will not fit in the specified set of data series.  In the above examples we have instructed chart to only draw 10 data series using the \code{limit 10} argument, this means that the chart module will first survey the data as it comes in to pick the best 9 data series to include and then add all other data series to the ``other'' bucket.  Chart calculates the value of the ``other'' data series using the exact same math as the rendered data series, often leading to the ``other'' data series dominating the output of a chart.
