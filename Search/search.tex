...
We will use the
\code{grep}\footnote{https://docs.gravwell.io/\#!search/grep/grep.md} module
twice, first filtering for the machine name and then filtering for the
service name. Our example query is:

\code{tag=syslog grep build \textbar{} grep sshd}

This query first tells all indexers that we only want to look at syslog
data (\code{tag=syslog}). The first \code{grep} search module filters out all entries which do not
have the value ``build'' in their data. The second \code{grep} filters out all
entries which do not have the value ``sshd'' in their data. The output
entries are guaranteed to contain both ``build'' and ``sshd'' in their
bodies.
...
Next, we'll use the \code{reimport} ingester to ingest a set of logs, 
...
Log into your Gravwell GUI (\href{http://localhost:8080}{http://localhost:8080}) and perform the following queries:

\begin{enumerate}
\item
  Filter the auth logs to only include logs from the sshd daemon on the
  host \code{porter}.
\item
  Filter the auth logs to include sshd logs from all hosts \emph{except} \code{porter}.
	\begin{enumerate}
	\item
	  The grep module has a -v option that inverts the filter logic
	\item
	  The grep module can also use glob patterns, e.g. \code{grep host*}
	\end{enumerate}
\item
  Filter auth logs to find successful logins on the host \code{porter}.
	\begin{enumerate}
	\item
	  sshd will log ``Accepted'' when a user logs in
	\end{enumerate}
\item
  ...
  accounts.
	\begin{enumerate}
	\item
	  Use the \code{count} module without any arguments to count entries
	\end{enumerate}
\end{enumerate}
...
\begin{figure}
	\includegraphics{images/extract-ip.png}
...
\begin{figure}
	\includegraphics{images/extract-chaining.png}
...
\begin{figure}
	\includegraphics{images/extract-ev.png}
...
\begin{figure}
	\includegraphics{images/hex-decode-url.png}
...
\begin{figure}
	\includegraphics{images/url-extract-json.png}
...
\item
  Filter to only include sshd messages by matching on the \code{app} enumerated value.
	\begin{enumerate}
	\item
	  Generate a table with the count of \code{app} events by each \code{host}.
	\end{enumerate}
\item
...
\begin{figure}
	\includegraphics{images/temporal-render.png}
...
\begin{figure}
	\includegraphics{images/nontemporal-render.png}
...
designed to show human readable entries in a text format. Any
...
Both renderers provide a useful feature when enumerated values have
been extracted from data. The bottom of the search results will show
buttons to switch between ``Entries'' view (Figure \ref{fig:text-entries}), which shows the original
entry contents, and ``Enumerated Values'' view (Figure \ref{fig:text-ev}), which shows the
enumerated values which were extracted from the entries.

\begin{figure}
	\includegraphics{images/text-entries.png}
	\caption{Text renderer entries view}
	\label{fig:text-entries}
\end{figure}

\begin{figure}
	\includegraphics{images/text-ev.png}
	\caption{Text renderer enumerated values view}
	\label{fig:text-ev}
\end{figure}

Hovering the mouse over an entry in the GUI when using either the text
or raw renderer brings up a context menu with three options, as shown
in Figure \ref{fig:text-context-menu}.

\begin{figure}
	\includegraphics{images/text-context-menu.png}
	\caption{Text renderer context menu}
	\label{fig:text-context-menu}
\end{figure}

The right-most icon shows the source and timestamp of the entry, and
the center icon copies the entry contents to the clipboard. The
left-most option, however, is particularly useful when dealing with
JSON-formatted data: it will attempt to pretty-print the entry's data as
JSON, as seen in Figure \ref{fig:text-json-prettyprint}.

\begin{figure}
	\includegraphics{images/text-json-prettyprint.png}
	\caption{Text renderer JSON pretty-print}
	\label{fig:text-json-prettyprint}
\end{figure}
...
\begin{Verbatim}[breaklines=true]
tag=syslog grep sshd | regex "authentication error for (?P<user>\S+)"
| count by user | table user count
\end{Verbatim}

\begin{figure}
	\includegraphics[width=0.5\linewidth]{images/table-brute-force.png}
	\caption{Brute force attempt table}
	\label{fig:table-brute-force}
\end{figure}
...
\begin{figure}
	\includegraphics{images/table-ssh-login.png}
...
The following query generates a chart showing which usernames most
commonly fail ssh authentication; due to online brute-forcing attacks,
we can expect "root" to be the most common.

\begin{Verbatim}[breaklines=true]
tag=syslog grep sshd | grep "Failed password for"
| regex "Failed\spassword\sfor\s(?P<user>\S+)"
| count by user | chart count by user limit 10
\end{Verbatim}
...
\begin{figure}
	\includegraphics{images/chart-line.png}
	\caption{Line chart}
	\label{fig:chart-line}
\end{figure}

\begin{figure}
	\includegraphics{images/chart-pie.png}
	\caption{Pie chart}
	\label{fig:chart-pie}
\end{figure}
...
The following search displays a map of all IP addresses captured in
netflow records, as seen in Figure \ref{fig:pointmap-ip}.
...
\begin{figure}
	\includegraphics{images/pointmap-ip.png}
	\caption{Pointmap IP locations}
	\label{fig:pointmap-ip}
\end{figure}

If we sum up the number of bytes from each IP and add the IP and Bytes
enumerated values to pointmap's arguments, they will appear when we
click on a point as shown in Figure \ref{fig:pointmap2}
(we also added the ASN Organization so we can see
\emph{who} we are talking to).

\begin{Verbatim}[breaklines=true]
tag=netflow netflow IP Bytes |
sum Bytes by IP |
geoip IP.Location |
geoip -r maxmindASN IP.ASNOrg |
pointmap IP Bytes ASNOrg
\end{Verbatim}

\begin{figure}
	\includegraphics{images/pointmap2.png}
...
location as the 'heat'. In this example using netflow records, the
'heat' represents the number of connections from a location, as seen
in Figure \ref{fig:heatmap-basic}.

\begin{Verbatim}[breaklines=true]
tag=netflow netflow IP | geoip IP.Lat IP.Long
| heatmap -lat Lat -long Long
\end{Verbatim}

\begin{figure}
	\includegraphics{images/heatmap-basic.png}
...
\begin{Verbatim}[breaklines=true]
tag=netflow netflow IP Bytes | sum Bytes by IP
| geoip IP.Location | heatmap sum
\end{Verbatim}

\begin{figure}
	\includegraphics{images/heatmap-bytes.png}
...
\begin{Verbatim}[breaklines=true]
tag=netflow netflow Src ~ PRIVATE Port < 1024 Bytes as traffic |
sum traffic by Src,Port | stackgraph Src Port sum
\end{Verbatim}

\begin{figure}[H]
	\includegraphics{images/stackgraph-traffic.png}
...
components of the bars indicate the different usernames given for failed SSH logins.
Note that China has been excluded, as they are the overwhelming majority of
attempts and make the other countries too small to read. Figure \ref{fig:stackgraph-ssh}
shows the results.

\begin{Verbatim}[breaklines=true]
tag=syslog grep sshd
| regex "Failed password for (?P<user>\S+) from (?P<ip>\d+\.\d+\.\d+\.\d+) "
| geoip ip.Country != CN | count by user,Country
| stackgraph Country user count
\end{Verbatim}

\begin{figure}[H]
	\includegraphics{images/stackgraph-ssh.png}
	\caption{Stackgraph of failed SSH logins by country \& user}
	\label{fig:stackgraph-ssh}
\end{figure}
...
\begin{figure}
	\includegraphics{images/fdg-classC.png}
...
\begin{figure}[H]
	\includegraphics{images/fdg-context.png}
...
\begin{figure}
	\includegraphics{images/resource-page.png}
...
\begin{figure}
	\includegraphics{images/resource-new.png}
...
\begin{figure}
	\includegraphics{images/resource-edit.png}
...
\begin{figure}[H]
	\includegraphics[width=0.7\linewidth]{images/maxmind-upload.png}
...
\begin{figure}[H]
	\includegraphics{images/maxmind-pointmap.png}
...
The \code{anko} search module allows users to write complex scripts to
...
In order to run an anko script in a search, a text file containing the
...
To run a script in the pipeline, simply specify \code{anko}, followed by the name of the
...
type \code{anko foo 1 3} in the pipeline.

\subsection{Writing anko scripts}

An anko script \emph{must} contain either a function named \code{Process} or
a function named \code{Main}. These functions do not take arguments. These
two functions represent two different options for processing search
entries.

If a \code{Process} function is defined, it will be called once per search
entry. Enumerated values on the entry may be treated as local variables,
and the return value of the \code{Process} function determines if the entry
is allowed to continue down the pipeline (true) or not (false).

If a \code{Main} function is defined, it will be called only once; the
programmer must therefore call the \code{readEntry} and
\code{writeEntry} functions to fetch each search entry and pass it down the
...
The script may also contain functions named \code{Parse} and
\code{Finalize}.

The \code{Parse} function is called before Process or Main and is
...
The \code{Finalize} function is called after Process or Main have
...
enumerated value we must use the \code{setEnum} function to make the action
explicit.

Note that the table and task variables are declared outside the
...
\code{functionName(\textless{}functionArgs\textgreater{})
\textless{}returnValues\textgreater{}}. The following functions can be
used in any anko script:

\begin{itemize}
\tightlist
\item
  \code{getResource(name) ([]byte, error)} - returns the slice of bytes
...
\item
  ...
  if one arises.
\item
  \code{producesEnum(val)} - Informs the pipeline that the script plans to produce an Enumerated Value of that name. Should be called in the Parse() function.
  \code{consumesEnum(val)} - Informs the pipeline that the script plans to consume the Enumerated Value of that name. Should be called in the Parse() function.
\item
...
\item
  \code{toInt(val) int64} - converts val to an integer if possible. Returns
...
\item
  \code{toBool(val) bool} - attempts to convert val to a boolean. Returns
...
\tightlist
\item
  \code{setEnum(key, value) error} - creates an enumerated value on the
  current entry named key containing value.
\item
  \code{getEnum(key) (value, error)} - returns the enumerated value
  specified by key.
\item
  \code{delEnum(key)} - deletes an enumerated value named key from the
  current entry.
\item
  \code{hasEnum(key) bool} - returns whether the current entry has the
  enumerated value.
\end{itemize}
...
\tightlist
\item
  \code{readEntry() (entry, error)} - returns the next entry and an error
  (if any). It will return an error when no entries remain.
\item
  \code{writeEntry(ent) error} - writes out the given entry down the
  pipeline, returning an error if any.
\item
  \code{cloneEntry(ent) entry} - returns a copy of the specified entry.
\item
  \code{newEntry() entry} - creates an entirely new entry, with the timestamp set to the current time.
\item
...
\item
  \code{hasEntryEnum(ent, key) bool} - returns whether the entry contains
  the enumerated value.
\item
  \code{delEntryEnum(ent, key)} - deletes the specified enumerated value
  from the given entry.
\item
  \code{setEntryData(ent, value)} - sets the data portion of an entry.
\item
  \code{setEntrySrc(ent, ip)} - sets the source field of an entry.
\item
  \code{setEntryTimestamp(ent, time)} - sets the timestamp of an entry.
\end{itemize}
...
Note the variables used: \code{SrcSub} and \code{DstPort}. Those \emph{must} be
...
The query uses the netflow module to extract the \code{Src} and
\code{DstPort} fields, then the subnet module finds the /8 subnet of the
Src IP and saves it to an enumerated value named \code{SrcSub}. Then
...
Note that entries with \code{DstPort == 443} have been marked ``https'',
while those with a \code{Src} in the 10.0.0.0/8 ubnet and \code{DstPort == 8080}
...
\begin{Verbatim}[breaklines=true]
tag=netflow netflow IP Bytes | stats sum(Bytes) by IP | table IP su
\end{Verbatim}
