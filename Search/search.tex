...
The ``other'' data group is a combined calculation of all remaining data groups that will not fit in the specified set of data series.  In the above examples we have instructed chart to only draw 10 data series using the \code{limit 10} argument, this means that the chart module will first survey the data as it comes in to pick the best 9 data sets to include and then add all other data sets will be combined into the ``other'' bucket.  Chart calculates the value of the ``other'' data set using the exact same math as the fully rendered data sets, often leading to the ``other'' data set dominating the output of a chart.
...
Notice that the ``other'' group represents the largest slice in the pie chart, indicating that the vast majority of usernames are not in the primary data sets.
